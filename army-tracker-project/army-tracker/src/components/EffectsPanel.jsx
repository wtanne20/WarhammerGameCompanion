  ...
  return (
    <section className="px-3 pt-5">
      ...
      <div className="mt-2 space-y-2 pl-2" style={{ borderLeft: `3px solid ${accent}` }}>
        {leader && (
          <div className="px-3 py-2" style={{ background: "#1E2228" }}>
            <div className="flex items-center justify-between gap-2">
              <span className="fs11 uppercase tracking-widest" style={{ color: "#8B929E" }}>Led by {leader.name}</span>
              <button onClick={() => onSetLeader(unit.instId, null)} className="p-1 active:opacity-60 shrink-0" style={{ color: "#8B929E" }}><Unlink size={14} /></button>
            </div>
            {leaderAbilities.length === 0 ? (
              <p className="fs11 mt-1" style={{ color: "#6B7280" }}>Check {leader.name}'s own sheet for anything that applies while attached.</p>
        ...
        {!editing && currentlyLeading && (
          <div className="px-3 py-2" style={{ background: "#1E2228" }}>
            <span className="fs11 uppercase tracking-widest" style={{ color: "#8B929E" }}>Leading {currentlyLeading.name}</span>
          </div>
        )}

        {editing && isCharacter && followers.length > 0 && (
          <div className="px-3 py-2" style={{ background: "#1E2228" }}>
            <span className="fs11 uppercase tracking-widest" style={{ color: "#8B929E" }}>Lead a unit</span>
              ...
              {followers.map((f) => {
                ...
                return (
                  <Chip key={f.instId} active={active} accent={accent}
                    ...
                    {otherLeader && <div className="fs9 mt-0.5" style={{ color: "#8B929E" }}>Led by {otherLeader.name}</div>}
                  </Chip>
                );
              })}
            </div>
        ...
        {editing && !leader && eligibleLeaders.length > 0 && (
          <div className="px-3 py-2" style={{ background: "#1E2228" }}>
            <span className="fs11 uppercase tracking-widest" style={{ color: "#8B929E" }}>Attach a leader</span>
            <div className="flex flex-wrap gap-2 mt-2">
        ...
        {isCharacter && detachment && detachment.enhancements.length > 0 && (editing || selectedEnhancement) && (
          <div className="px-3 py-2" style={{ background: "#1E2228" }}>
            <span className="fs11 uppercase tracking-widest" style={{ color: "#8B929E" }}>Enhancement</span>
            {editing && (
              <div className="flex flex-wrap gap-2 mt-2">
                ...
                {detachment.enhancements.map((e) => (
                  <Chip key={e.id} active={unit.enhancementId === e.id} accent={accent} onClick={() => onSetEnhancement(unit.instId, e.id)}>
                    ...
                    <div className="fs10 tnum mt-0.5" style={{ color: unit.enhancementId === e.id ? "rgba(232,226,212,0.8)" : "#8B929E" }}>{e.points} pts</div>
                  </Chip>
                ))}
              </div>
            )}
            {selectedEnhancement && (
              <div className="mt-2">
                ...
                <span style={{ color: "#C5C9D0", whiteSpace: "pre-line" }}>{selectedEnhancement.text}</span>
              </div>
            )}
          </div>
        )}
        ...
        {matchedAbilities.map((a, i) => (
          <div key={i} className="px-3 py-2" style={{ background: "#1E2228" }}>
            <div className="fs10 uppercase tracking-widest" style={{ color: "#8B929E" }}>Possible detachment bonus — verify</div>
            <span className="font-semibold" style={{ color: "#B8925A" }}>{a.name}.</span>{" "}
            <span style={{ color: "#C5C9D0", whiteSpace: "pre-line" }}>{a.text}</span>
          </div>
        ))}

        {customEffects.map((ce) => (
          <div key={ce.id} className="px-3 py-2 flex items-center justify-between gap-2" style={{ background: "#1E2228" }}>
            <span style={{ color: "#C5C9D0" }}>{ce.text}</span>
            <button onClick={() => onRemoveCustomEffect(unit.instId, ce.id)} className="p-1 active:opacity-60 shrink-0" style={{ color: "#8B929E" }}><X size={14} /></button>
          </div>
        ))}

        <div className="px-3 py-2 flex items-center gap-2" style={{ background: "#1E2228" }}>
          <input value={note} onChange={(e) => setNote(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && addNote()}
            placeholder="Add a note (battle-shocked, stratagem, etc.)"
            className="flex-1 bg-transparent outline-none text-sm py-1" style={{ color: "#E8E2D4" }} />
