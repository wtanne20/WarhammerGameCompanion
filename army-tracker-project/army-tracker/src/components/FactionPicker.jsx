    ...
    <div className="fixed inset-0 z-20 flex flex-col justify-end" style={{ background: "rgba(0,0,0,0.6)" }} onClick={onClose}>
      <div className="max-w-xl w-full mx-auto overflow-y-auto" style={{ background: "var(--wh-bg)", borderTop: "2px solid var(--wh-accent)", maxHeight: "85vh" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-4 pb-6 space-y-2">
          {results.length === 0 && (
            <div className="text-center py-10 fs11 uppercase tracking-widest" style={{ color: "var(--wh-muted)" }}>No factions match "{q}"</div>
          )}
          {results.map((f) => {
            const accent = factionAccent(f);
            return (
              <button key={f} onClick={() => onPick(f)} className="w-full text-left flex items-center gap-3 px-4 py-3 active:opacity-80" style={{ background: "var(--wh-surface)" }}>
                <div className="shrink-0 flex items-center justify-center font-display uppercase text-sm"
                  style={{ width: 32, height: 32, background: accent, color: "var(--wh-text)" }}>
                  {f.charAt(0)}
                </div>
