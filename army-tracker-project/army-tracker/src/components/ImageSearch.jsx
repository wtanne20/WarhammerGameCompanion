      ...
      <div className="max-w-xl w-full mx-auto overflow-y-auto" style={{ background: "var(--wh-bg)", borderTop: "2px solid var(--wh-accent)", maxHeight: "85vh" }} onClick={(e) => e.stopPropagation()}>
        <div className="sticky top-0 z-10" style={{ background: "var(--wh-bg)" }}>
          ...
          <div className="px-4 pb-3">
            ...
            <p className="fs10 mt-2" style={{ color: "var(--wh-dim)" }}>
        ...
        <div className="px-4 pb-6">
          {status === "loading" && (
            <div className="flex items-center justify-center py-10" style={{ color: "var(--wh-muted)" }}>
          ...
          {matchedQuery && (
            <p className="fs10 uppercase tracking-widest pb-2" style={{ color: "var(--wh-muted)" }}>
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
              {results.map((r) => (
                <button key={r.id} onClick={() => pick(r)} disabled={downloadingId === r.id}
                  className="relative text-left overflow-hidden active:opacity-80" style={{ background: "var(--wh-surface)", aspectRatio: "1 / 1" }}>
                  <img src={r.thumbnail} alt={r.title} loading="lazy" className="absolute inset-0 w-full h-full object-cover" />
                  {(r.creator || r.title) && (
                    <div className="absolute inset-x-0 bottom-0 px-2 py-1 fs9 truncate" style={{ background: "rgba(15,17,21,0.85)", color: "var(--wh-text-body)" }}>
                      {r.creator || r.title}
                    </div>
                  )}
                  {downloadingId === r.id && (
                    <div className="absolute inset-0 flex items-center justify-center" style={{ background: "rgba(15,17,21,0.75)" }}>
                      <Loader2 className="animate-spin" size={20} style={{ color: "var(--wh-text)" }} />
                    </div>
                  )}
                  {downloadError === r.id && (
