      ...
      <div className="max-w-xl w-full mx-auto overflow-y-auto" style={{ background: "var(--wh-bg)", borderTop: "2px solid var(--wh-accent)", maxHeight: "85vh" }} onClick={(e) => e.stopPropagation()}>
        ...
        {preview ? (
          <div className="px-4 pb-6">
            <div className="px-4 py-3 mb-3" style={{ background: "var(--wh-surface)" }}>
            ...
            <div className="flex gap-2">
              <button onClick={() => setPreview(null)} className="flex-1 flex items-center justify-center gap-2 py-3.5 fs11 uppercase tracking-widest active:opacity-70" style={{ background: "var(--wh-surface)", color: "var(--wh-muted)" }}>
                <ArrowLeft size={16} /> Back
              </button>
              <button onClick={() => onImport(preview)} className="flex-1 py-3.5 font-display uppercase tracking-widest text-sm active:opacity-70" style={{ background: "var(--wh-accent)", color: "var(--wh-text)" }}>
                Import
              </button>
            </div>
          </div>
          ...
          <div className="px-4 pb-6">
            <textarea value={pasteText} onChange={(e) => setPasteText(e.target.value)} rows={8} placeholder="Paste the exported army JSON here…"
              className="w-full outline-none p-3 text-sm resize-none mb-3" style={{ background: "var(--wh-surface)", color: "var(--wh-text)" }} />
            {error && <p className="fs11 mb-3" style={{ color: "#C97B7B" }}>{error}</p>}
            <div className="flex gap-2">
              <button onClick={() => { setPasting(false); setError(null); }} className="flex-1 flex items-center justify-center gap-2 py-3.5 fs11 uppercase tracking-widest active:opacity-70" style={{ background: "var(--wh-surface)", color: "var(--wh-muted)" }}>
                <ArrowLeft size={16} /> Back
              </button>
              <button onClick={() => handleParsed(pasteText)} className="flex-1 py-3.5 font-display uppercase tracking-widest text-sm active:opacity-70" style={{ background: "var(--wh-accent)", color: "var(--wh-text)" }}>
                Preview
              </button>
            </div>
          </div>
          ...
          <div className="px-4 pb-6 space-y-2">
            ...
            <button onClick={() => fileInputRef.current?.click()} className="w-full flex items-center gap-3 px-4 py-3.5 text-left active:opacity-80" style={{ background: "var(--wh-surface)" }}>
              <span style={{ color: "var(--wh-accent-gold)" }}><Upload size={18} /></span>
              <span className="font-display uppercase tracking-wide text-sm">Choose a file</span>
            </button>
            <button onClick={() => setPasting(true)} className="w-full flex items-center gap-3 px-4 py-3.5 text-left active:opacity-80" style={{ background: "var(--wh-surface)" }}>
              <span style={{ color: "var(--wh-accent-gold)" }}><ClipboardPaste size={18} /></span>
