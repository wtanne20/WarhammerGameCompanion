        ...
        <div className="sticky top-0 z-10" style={{ background: "var(--wh-bg)" }}>
          ...
          <div className="px-4 pb-3">
            <input autoFocus value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search primary missions…"
              className="w-full outline-none py-3 px-3 text-sm" style={{ background: "var(--wh-surface)", color: "var(--wh-text)" }} />
          </div>
        </div>

        <div className="px-4 pb-6 space-y-4">
          {results.length === 0 && (
            <div className="text-center py-10 fs11 uppercase tracking-widest" style={{ color: "var(--wh-muted)" }}>No missions match "{q}"</div>
          )}
          {groups.map(([format, missionsInFormat]) => (
            <div key={format}>
              <div className="fs10 uppercase tracking-widest mb-2" style={{ color: "var(--wh-dim)" }}>{format}</div>
              <div className="space-y-2">
                {missionsInFormat.map((m) => (
                  <button key={m.id} onClick={() => onPick(m.id)} className="w-full text-left px-4 py-3 active:opacity-80" style={{ background: "var(--wh-surface)" }}>
                    <div className="font-display uppercase tracking-wide text-base">{m.name}</div>
                    <div className="fs11 mt-1" style={{ color: "var(--wh-muted)" }}>{m.description}</div>
                  </button>
                ))}
              </div>
            </div>
